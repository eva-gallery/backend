  ...
  async getRandomArtists(seed: number, from: number = 0, count: number = 1) {
    ...
    return this.artists.manager.transaction(async mgr => {
      ...
      const res = items.entities.map((artist, i) => {
        ...
        const computedProps = {
          get imageFilename() { 
            return `${artwork.image_hash}.${getExtensionForMimeType(artwork.image_mime_type)}`;
          },
          get thumbnailFilename() {
            return `${artwork.image_hash}.${getExtensionForMimeType(artwork.thumbnail_mime_type)}`;
          }
        };
